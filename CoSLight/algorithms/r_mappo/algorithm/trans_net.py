...
class TransformerEncoderModel(nn.Module):
    ...
    def forward(self, src, src_mask=None): # src.shape : T, B, x  T=1 timestep, B=agent number;; src_mask:16,8
        ...
        x = self.transformer_encoder(x_pos, src_mask)  ### Transformer input S (sequence length, here refers to the number of agents), N (batchsize, here is 1 moment), F (feature dimension)
        #  
    ...
    def compute_score(self, src, src_mask=None): # src.shape : T, B, x  This refers to T=1 time step, B=agent number
        ...
        src_mask = src_mask.repeat(self.args.trans_heads, 1, 1)
        h, attn = self.transformer_encoder(x_pos_, src_mask, output_att=True)  ### Transformer input S (sequence length, here refers to the number of agents), N (batchsize, here is 1 moment), F (feature dimension)
        #  
