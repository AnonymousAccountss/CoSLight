...
from onpolicy.algorithms.r_mappo.algorithm.advanced_net import *
...
class R_Actor_Advanced(nn.Module):
    ...
    def __init__(self, args, obs_space, action_space, device=torch.device("cpu")):
        ...
        # self.hidden_size = 64
        self.model = ModelBody(1, args.hidden_layer_size, self.args.state_key, device=device, args=args).to(device)
        ...
        if args.use_trans_hidden:   
            ...
            self.hidden_size = 2 * 7 * 8 # 7个特征， 8个phase
    ...
    def forward(self, obs, rnn_states, masks, available_actions=None, deterministic=False, trans_masks=None):
        ...
        actor_features_ = self.model(obs)  # 2.8.32
        ...
        actor_features_ones = torch.ones_like(actor_features, device=actor_features.device) 
        actor_features_new = torch.cat((actor_features_ones, actor_features_ones), dim=-1)
    ...
    def evaluate_actions(self, obs, rnn_states, action, masks, available_actions=None, active_masks=None, score_batch=None, trans_masks=None):
        ...
        actor_features_ = self.model(obs)  # 20.8.64
        
        if len(actor_features_.shape) == 3: # reshape 成两维
            ...
            score_action_log_probs = action_log_probs
            score_dist_entropy = dist_entropy
...
class R_Critic(nn.Module):
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        if args.use_trans_hidden:   
            ...
            trans_hidden = 2 * 7 * 8 # 7个特征， 8个phase
...
class R_Critic_Trans_all(nn.Module):
    ...
    def forward(self, obs, rnn_states, masks, topk_index, backward=False):
        ...
        for i_agent in range(self.args.num_agents):
            ...
            value, rnn_state = self.critics[i_agent](obs_i, masks_i, rnn_states_i, backward)  ### what ?? 消失的 rnn_states_i？？
            values.append(value)
