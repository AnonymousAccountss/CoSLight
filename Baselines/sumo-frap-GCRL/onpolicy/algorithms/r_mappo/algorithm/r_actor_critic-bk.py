...
class R_Actor_Trans(nn.Module):
    ...
    def forward(self, obs, rnn_states, masks, available_actions=None, deterministic=False, trans_masks=None):
        ...
        if self.args.fix_matrix == 0: # # fix_matrix=0 : 固定矩阵; # fix_matrix=1 : 随机矩阵; 
            row = torch.arange(self.args.use_K, device=scores.device)
            scores_new = row.repeat(self.args.num_agents*self.args.n_rollout_threads, 1)
        elif self.args.fix_matrix == 1: # # fix_matrix=0 : 固定矩阵; # fix_matrix=1 : 随机矩阵; 
            scores_new = torch.randint(0, self.args.num_agents, scores.shape, device=scores.device)
        else:
            scores_new = scores 

        scores_ = scores_new.reshape(-1, self.args.num_agents, self.args.use_K) # 2.8.5
