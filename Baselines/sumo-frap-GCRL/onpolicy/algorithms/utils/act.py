...
class ACTLayer_TopK(nn.Module):
    ...
    def __init__(self, action_space, inputs_dim, use_orthogonal, gain, args=None):
        ...
        if action_space.__class__.__name__ == "Discrete":
            action_dim = action_space.n
            self.action_out = Categorical_Topk(inputs_dim, action_dim, use_orthogonal, gain, args)
        elif action_space.__class__.__name__ == "Box":
            action_dim = action_space.shape[0]
            self.action_out = DiagGaussian(inputs_dim, action_dim, use_orthogonal, gain)
        elif action_space.__class__.__name__ == "MultiBinary":
    ...
    def evaluate_actions(self, x, action, available_actions=None, active_masks=None):
        ...
        if self.mixed_action:
            ...
            if active_masks is not None:
                dist_entropy = (self.action_out.entropy()*active_masks.squeeze(-1)).sum()/active_masks.sum()
            else:
