...
class SUMOBaseRunner(object):
    ...
    def __init__(self, config):
        ...
        # algorithm
        self.trainer = TrainAlgo(self.all_args, self.policy, self.all_args.num_agents, device = self.device)
     
        # if self.model_dir is not None:
        #     self.restore()
        
        if self.model_dir is not None and self.all_args.not_update:
            self.restore(self.all_args.model_id)
    ...
    def compute(self):
        ...
        hidden_layer = check(np.concatenate(self.buffer.obs[-1])).to(**self.tpdv)
        # hidden_states = check(np.concatenate(self.buffer.hidden_states[-1])).to(**self.tpdv)
        next_values = self.trainer.policy.get_values(hidden_layer,
    ...
    def save(self, episode):
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            if self.trainer._use_valuenorm:
                ...
                torch.save(policy_vnorm.state_dict(), str(self.save_dir) + "/vnorm_{}.pt".format(episode))

    def restore(self, id_):
        """Restore policy's networks from a saved model."""
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            self.policy.restore(str(self.model_dir) + f'/transformer_{id_}.pt')
        else:
            policy_actor_state_dict = torch.load(str(self.model_dir) + f'/actor_{id_}.pt')
            self.policy.actor.load_state_dict(policy_actor_state_dict)
            if not self.all_args.use_render:
                policy_critic_state_dict = torch.load(str(self.model_dir) + f'/critic_{id_}.pt')
                self.policy.critic.load_state_dict(policy_critic_state_dict)
                if self.trainer._use_valuenorm:
                    policy_vnorm_state_dict = torch.load(str(self.model_dir) + f'/vnorm_{id_}.pt')
