...
class SUMORunner(SUMOBaseRunner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                print('-----step', step)
                # Sample actions
                values, actions, action_log_probs, rnn_state, rnn_state_critic = self.collect(step)
                ...
                data = obs, rewards, dones, infos, values, actions[:,:,None].astype(np.int64), action_log_probs[:,:,None], available_actions,rnn_state, rnn_state_critic
            ...
            if not self.all_args.not_update:        
                ...
                if self.args.model_dir is None and not self.args.not_update:
                    self.compute()
    ...
    def collect(self, step):
        ...
        rnn_state_critic = np.array(np.split(_t2n(rnn_state_critic), self.n_rollout_threads))
        # rearrange action
    ...
    def insert(self, data):
        obs, rewards, dones, infos, values, actions, action_log_probs, available_actions, rnn_state, rnn_state_critic = data
        ...
        self.buffer.insert(share_obs, obs, rnn_state, rnn_state_critic, actions, action_log_probs, values, rewards, masks, available_actions=available_actions)

    @torch.no_grad()
    def eval(self, total_num_steps):
        eval_episode_rewards = []
        eval_obs = self.eval_envs.reset()
        eval_share_obs = eval_obs.reshape(self.n_rollout_threads, -1)
