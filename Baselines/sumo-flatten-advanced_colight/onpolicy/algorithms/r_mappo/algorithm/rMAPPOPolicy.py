import torch
from onpolicy.algorithms.r_mappo.algorithm.r_actor_critic import R_Actor, R_Critic, R_Actor_Advanced, R_Critic_Trans, R_Critic_Trans_all, R_Critic_all
...
class R_MAPPOPolicy_Advanced:
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None,
        ...
        if self.args.model_dir is not None and self.args.not_update:
            deterministic = True
        ...
        #     obs_ = np.array(np.split(obs, roll_outs))
        #     obs_ = check(obs_).to(**self.tpdv)
        #     atten_scores = self.actor.model.compute_score(obs_)  ## 2.8.8
        #     batch_size_obs = obs_.shape[0]
        #     topk_index = torch.topk(atten_scores, k=self.args.use_K, dim=-1)[1] # 取出前五对应的index  20.16.5
        #     # # 生成 batch 索引 
        #     batch_indices = torch.arange(self.args.num_agents)[None,:,None].repeat(batch_size_obs, 1, 1).to(self.device) # 20.16.1
        #     topk_index = torch.cat((topk_index, batch_indices), dim=-1)  ### 注意这里的顺序，因为要取当前智能体的对应的trans的输出，所以放在后面
                        
        #     values, rnn_states_critic = self.critic(cent_obs, rnn_states_critic, masks, topk_index.cpu())

        values, rnn_states_critic = self.critic(actor_features_new[:,:self.args.trans_hidden], rnn_states_critic, masks)
    ...
    def get_values_three(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None,
        ...
        actions, action_log_probs, rnn_states_actor, actor_features, scores, score_log_probs = self.actor(obs,
                                                                 ...
                                                                 deterministic,
                                                                 trans_masks=trans_masks)
        values, _ = self.critic(actor_features[:,:self.args.trans_hidden], rnn_states_critic, masks)
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, action, masks,
        ...
        if len(obs.shape) == 3:
            ...
            score_batch = score_batch.reshape(-1, *score_batch.shape[2:])
        ...
        values, _ = self.critic(actor_features_batch[:,:,:self.args.trans_hidden], rnn_states_critic, masks, backward=True)
