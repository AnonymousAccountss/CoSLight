...
class SharedReplayBufferSUMO(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.hidden_layer = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, 16, 8, 7), dtype=np.float32)
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        #                                                                                   3:])
        hidden_layer = self.hidden_layer[:-1].transpose(1, 2, 0, 3, 4, 5).reshape(-1, *self.hidden_layer.shape[3:])
    ...
    def recurrent_generator_graph(self, advantages, num_mini_batch, data_chunk_length):
        ...
        # hidden_layer = self.hidden_layer[:-1].transpose(1, 2, 0, 3, 4, 5).reshape(-1, *self.hidden_layer.shape[3:]) # (40, 2, 8, 16, 8, 7) --> (640, 16, 8, 7)
        hidden_layer = self.hidden_layer[:-1].transpose(2, 1, 0, 3, 4, 5).reshape(num_agents, -1, *self.hidden_layer.shape[3:]).transpose(1, 0, 2, 3, 4) # (40, 2, 8, 16, 8, 7) --> (80, 8, 16, 8, 7)
